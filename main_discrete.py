    ...
    for episode in range(args.max_episode+1):
        ...
        while not done and i <= args.max_iterations:
            ...
            if position < next_position:
                reward_pos = (next_position - position)/i
                if position > 0.9 * env.img_width:
                    reward_pos = 10
            else:
                reward_pos = -1
        ...
        reward_avg = [action_sum/(i*np.sign(init_state[2])), abs(state[2] - init_state[2])/i]
        ...
        if episode % 50 == 0:
            ...
            print(f"Avg reward: {reward_avg}; Final state: {state}; Initial state: {init_state}; "
                  f"Final step reward: {[reward_pos, reward_error]}; Times of iters: {i}")
