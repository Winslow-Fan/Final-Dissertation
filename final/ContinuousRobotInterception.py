...
class TwoWheelRobotContinuousMovingEnv(gym.Env):
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        self.middle = 0
        # Action space
        self.action_space = spaces.box(low=np.array([-1, -1]), high=np.array([1, 1]), dtype=int)
    ...
    def step(self, action: int):
        ...
        bbox_width = int(self.img_width * ((up_bound - down_bound) % tau) / self.view_range)
        bbox_bias = int(self.img_width / 2 - bbox_middle)
        bbox_height = int(self.angle_target * self.img_height / self.view_range * 1.6)
        self.state = np.array([bbox_middle, bbox_width, bbox_bias, bbox_height]).astype(int)

        reward = 0

        self.last_state = self.state
        if abs(bbox_bias) <= 1 or bbox_width >= 0.9 * self.img_width: self.middle += 1
        done = bool((self.middle >= 5 and bbox_height >= 0.9 * self.img_height) or bbox_height >= 0.9 * self.img_height)
        if done:
            self.reward = 2000
        else:
            self.reward = reward
